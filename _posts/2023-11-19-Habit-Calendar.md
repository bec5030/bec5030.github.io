...
The through holes under the button and LED components are highlighted in the image below.
--<through hole photo>



The boards are designed to tile. Physically, they are spaced so that they can tile in any direction, but electrically, they can only tile up and down as all the connections are routed to the top and bottom of the board. The bottom board pins can be disconnected by removing zero ohm resistors.
Power and ground must go through these connections, but the rest may be optional if no master controller is required.
...
The actual boards can be seen here:
...
--<instert settings and state change controls photo>
